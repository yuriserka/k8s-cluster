...
Uses the official [localstack/helm-charts](https://github.com/localstack/helm-charts) chart. Auth token is **not** stored in `values.yaml`; [`install_infra.py`](../../../scripts/install_infra.py) reads `LOCALSTACK_AUTH_TOKEN` from [`resources/vault/_admin/aws/dev/.env`](../../../resources/vault/_admin/aws/dev/.env) at install time.
...
# plus LOCALSTACK_AUTH_TOKEN from vault — use install_infra.py
```
...
```bash
cd scripts
python install_infra.py --namespace dev
```
...
|--|--|--|
| Start | `cd apps/infra && docker compose up -d` or `install_infra.py --mode compose` | `make setup-infra` |
