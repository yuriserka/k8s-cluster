...
class S3MultipartUploaderService:
    async def upload_file(
        ...
        try:
            parts = await self._upload_file_parts(
                s3_client, data_stream, bucket, key, upload_id
            )
            ...
            logger.error(
                f"[S3MultipartUploaderService] failed to complete mpu for {key} "
                f"// {upload_id}. {e}"
            )
    ...
    async def _upload_file_parts(
        ...
        async for chunk in data_stream:
            ...
            while len(buffer) >= PART_SIZE:
                ...
                logger.info(
                    f"[S3MultipartUploaderService] part {part_number} "
                    f"for {key} // {upload_id}"
                )
        ...
        if buffer:
            ...
            logger.info(
                f"[S3MultipartUploaderService] final part {part_number} "
                f"for {key} // {upload_id}"
            )
            parts.append({
                'ETag': part['ETag'],
