...
def get_database_admin_credentials(namespace: str):
    env_dir = os.path.join(
        REPO_ROOT, 'resources', 'vault', '_admin', 'database', namespace
    )
...
def get_database_credentials(repository: str, namespace: str):
    env_dir = os.path.join(
        REPO_ROOT, 'resources', 'vault', repository, 'database', namespace
    )
    if not os.path.isdir(env_dir):
        raise FileNotFoundError(
            f'Environment directory {env_dir} does not exist.')
...
def kubectl_exec_psql(namespace, admin_user, admin_password, sql, database=None):
    ...
    inner = ' '.join(parts)
    execute_cli_command(
        'minikube kubectl -- exec -it postgresql-0 -n '
        f'{shlex.quote(namespace)} -- bash -c {shlex.quote(inner)}'
    )
...
def grant_database_privileges(
    ...
    kubectl_exec_psql(
        ...
        admin_password,
        (
            f'ALTER DATABASE {quoted_db} OWNER TO {app_user}; '
            f'GRANT CONNECT ON DATABASE {quoted_db} TO {app_user};'
        ),
    )
...
def main(repository: str, namespace: str):
    ...
    if not all([admin_password, admin_user, db_name, app_user]):
        raise ValueError(
            'Missing required database credentials (admin USER/PASSWORD, app NAME/USER).'
        )

    print(f'Creating database "{db_name}" with owner "{app_user}" (if not exists)...')
    create_database_if_missing(
        namespace, admin_user, admin_password, db_name, app_user
    )
    ...
    print(
        f'Database "{db_name}" is ready in namespace "{namespace}".')
    print(
        f'To connect, use: {db_to_create_credentials}')
    print(
